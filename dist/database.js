    ...
    articles: [
        {
            const response = await fetch(`${this.baseURL}/articles/${id}`, {
                method: 'DELETE',
                credentials: 'include',
                headers: { 'X-CSRF-Token': (typeof sessionStorage !== 'undefined' ? sessionStorage.getItem('csrf-token') : '') || '' }
            });
...
class MTACMNMySQLClient {
    ...
    async createArticle(articleData) {
        try {
            const response = await fetch(`${this.baseURL}/articles`, {
                method: 'POST',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': (typeof sessionStorage !== 'undefined' ? sessionStorage.getItem('csrf-token') : '') || '' },
    ...
    async deleteArticle(id) {
        try {
            ...
            // include CSRF header for DELETE
            // browsers don't allow headers in simple fetch if no body; we create explicit init above
